...
numSamples1 = sum(iris_labels == 0); % Number of samples in Class 0
numSamples2 = sum(iris_labels == 1); % Number of samples in Class 1
numSamples3 = sum(iris_labels == 2); % Number of samples in Class 2

fprintf('Number of samples in Class 0: %d\n', numSamples1);
...
figure(3)
hold on
plot(IRIS1(:, 1), IRIS1(:, 2), 'bo', 'MarkerFaceColor', 'b'); % Class 0 (setosa) - blue circles
plot(IRIS2(:, 1), IRIS2(:, 2), 'rs', 'MarkerFaceColor', 'r'); % Class 1 (versicolor) - red squares
plot(IRIS3(:, 1), IRIS3(:, 2), 'g+', 'MarkerFaceColor', 'g'); % Class 2 (virginica) - green plus signs
hold off
...
% Project the data onto the direction of the two-dimensional v
[meas_reduced] = projectDataLDA(meas_norm, v);

% Get the data for each class in the reduced space
IRIS4 = meas_reduced(iris_labels == 0, :); % Samples of Class 0
IRIS5 = meas_reduced(iris_labels == 1, :); % Samples of Class 1
IRIS6 = meas_reduced(iris_labels == 2, :); % Samples of Class 2

% Visualize the sample dataset after LDA is applied
% Use different color/symbol for each class
figure(4)
hold on
plot(IRIS4(:, 1), IRIS4(:, 2), 'bo', 'MarkerFaceColor', 'b'); % Class 0 (setosa) - blue circles
plot(IRIS5(:, 1), IRIS5(:, 2), 'rs', 'MarkerFaceColor', 'r'); % Class 1 (versicolor) - red squares
plot(IRIS6(:, 1), IRIS6(:, 2), 'g+', 'MarkerFaceColor', 'g'); % Class 2 (virginica) - green plus signs
hold off

xlabel('LDA Dimension 1');
ylabel('LDA Dimension 2');
title('2D Visualization after LDA');
legend('Class 0 (setosa)', 'Class 1 (versicolor)', 'Class 2 (virginica)');
